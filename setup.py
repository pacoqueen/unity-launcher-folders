...
try:
    ...
    print >> sys.stderr, 'To build unity-launcher-folders you need https://launchpad.net/python-distutils-extra'
    sys.exit(1)
assert DistUtilsExtra.auto.__version__ >= '2.18', 'needs DistUtilsExtra.auto >= 2.18'

def update_config(libdir, values = {}):

    filename = os.path.join(libdir, 'unity_launcher_folders_lib/unity_launcher_foldersconfig.py')
    oldvalues = {}
    try:
        fin = file(filename, 'r')
...
def move_desktop_file(root, target_data, prefix):
    ...
    old_desktop_path = os.path.normpath(root + target_data +
                                        '/share/applications')
    old_desktop_file = old_desktop_path + '/unity-launcher-folders.desktop'
    desktop_path = os.path.normpath(root + prefix + '/share/applications')
    desktop_file = desktop_path + '/unity-launcher-folders.desktop'
    ...
    if not os.path.exists(old_desktop_file):
        ...
        desktop_file = desktop_path + '/extras-unity-launcher-folders.desktop'
        try:
...
def update_desktop_file(filename, target_pkgdata, target_scripts):
    ...
    try:
        ...
        for line in fin:
            if 'Icon=' in line:
                line = "Icon=%s\n" % (target_pkgdata + 'media/unity-launcher-folders.png')
            elif 'Exec=' in line:
                cmd = line.split("=")[1].split(None, 1)
                line = "Exec=%s" % (target_scripts + 'unity-launcher-folders')
                if len(cmd) > 1:
...
class InstallAndUpdateDataDirectory(DistUtilsExtra.auto.install_auto):
    def run(self):
        ...
        target_pkgdata = target_data + 'share/unity-launcher-folders/'
        target_scripts = '/' + os.path.relpath(self.install_scripts, self.root) + '/'

        values = {'__unity_launcher_folders_data_directory__': "'%s'" % (target_pkgdata),
...
DistUtilsExtra.auto.setup(
    name='unity-launcher-folders',
    version='14.09.5',
    license='GPL-3',
    author='Anton Sukhovatkin',
    author_email='laucnherfolders@exceptionfound.com',
    description='Folders in Unity Launcher to store files and/or apps',
    long_description='Create a folder in Unity Launcher to store shortcuts to: apps, links from chrome or firefox, files, folders, create google chrome web apps, shell scripts, etc',
    url='https://launchpad.net/unity-launcher-folders',
